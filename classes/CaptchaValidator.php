...
/**
 * Drop-in override of the LHC core CaptchaValidator that adds 'pow' as a
 * first-class provider.  Loaded via a prepended autoloader in bootstrap.php
 * before the core file is ever required, so the rest of LHC (login, forgot-
 * password, chat widgets …) automatically gains PoW support without any
 * module-level overrides.
 *
 * Google reCAPTCHA v3 and Cloudflare Turnstile logic is kept byte-for-byte
 * identical to the core implementation so this file can be diffed easily
...
class CaptchaValidator
{
    ...
    public static function validateAuthCaptcha(array $postData, string $context): array
    {
        ...
        if ($captchaSettings['provider'] === 'pow') {
            $reason = null;
            if (\erLhcoreClassPowCaptcha::verifySubmittedProof($postData, $context, $reason)) {
                return array('valid' => true, 'provider' => 'pow', 'reason' => 'validated');
            }
            return array('valid' => false, 'provider' => 'pow', 'reason' => $reason ?? 'invalid_proof');
        }
